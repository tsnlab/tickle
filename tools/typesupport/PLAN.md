...
compatibility, XCDR2, type hashes, introspection typesupport, a *bounded* string element inside
an array (`string<=N[]`/`string<=N[<=M]`), arrays of nested message types, and defaults on a
nested message field. `adapt.py` raises a clear error for each rather than silently generating
something wrong.
