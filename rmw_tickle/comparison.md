...
| DEADLINE | **stale note corrected (2026-09-20)**: now native at the wire/RxO level - `tt_Publisher.deadline_duration_ns`/`tt_Subscriber.deadline_duration_ns` (Milestone 49) are real, wired fields feeding real RxO incompatibility matching (`subscriber_incompatible_with_publisher()`, `tickle.c`). Still **not** enforced/detected by core itself, exactly as before - `tickle.h`'s own doc comment says so directly ("TickLE core itself never enforces or checks this on its own... the rmw layer already does that independently") - a miss-detection HIL example still has to do that check itself, the same as before this milestone landed. |
| (discovery/departure, used for LIVELINESS loss detection) | `tt_Node_set_discovery()` + `tt_DISCOVERY_CALLBACK(node, node_id, endpoint_id, kind, departed, param)` - fires `departed=true` on `check_liveliness()`'s own timeout. **Node-level, not per-Publisher**: the detection window is the fixed `tt_LIVELINESS_MISS_THRESHOLD * tt_NODE_UPDATE_INTERVAL` (~3s, `config.h`), not `liveliness_lease_duration_ns`'s own per-entity announced value (that field is wire/RxO-only, same "announced but not core-enforced" shape as DEADLINE above) - a departed *node* takes every Publisher/Subscriber it hosted down with it, there's no finer-grained single-entity liveliness loss below that. |
