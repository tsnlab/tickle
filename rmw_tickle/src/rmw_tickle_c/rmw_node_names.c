...
rmw_ret_t rmw_get_node_names(const rmw_node_t* node, rcutils_string_array_t* node_names,
    ...
    if (RCUTILS_RET_OK != rcutils_string_array_init(node_names, discovered_node_count, &allocator)) {
        RMW_SET_ERROR_MSG("Failed to initialize node_names string array");
        return RMW_RET_ERROR;
    }

    if (RCUTILS_RET_OK != rcutils_string_array_init(node_namespaces, discovered_node_count, &allocator)) {
    ...
    for (int i = 0; i < TICKLE_MAX_ENDPOINT_COUNT && node_index < discovered_node_count; i++) {
        ...
        node_names->data[node_index] = allocator.zero_allocate(name_len, sizeof(char), allocator.state);
        ...
        node_namespaces->data[node_index] = allocator.zero_allocate(namespace_len, sizeof(char), allocator.state);
