...
// the whole sequence; runs for SECONDS. client: sends a goal of order 10, then checks every feedback
// is a prefix of the true sequence and the result is exactly it. Exit 0 on success; 1 on a wrong
...
namespace {
    ...
    auto request(const rclcpp::Node::SharedPtr& node, long seconds) -> int {
        ...
        const bool pass = result_ok && feedback_ok && feedbacks > 0;
        std::printf("action client: %s\n", pass ? "PASS - Fibonacci round-tripped as an action" : "FAIL");
        return pass ? 0 : 1;
    }
} // namespace
