...
| **1** | `rosidl_typesupport_tickle_c`: (a) ✅ `tickle_typesupport.ros2_adapter` - generates a converter between a real ROS 2 interface package's own `rosidl_generator_c` struct and TickLE's already-generated, already-tested struct/codec for that message (field-by-field copy + bounds checks; reuses `tools/typesupport`'s existing CDR-4 codec completely unchanged - see the module's own docstring for the exact `rosidl_runtime_c` field mapping assumed). Verified fully offline (no ROS 2 install in this tool's own dev/test environment - `tests/fixtures_ros2_adapter/`'s hand-written stand-ins for `rosidl_generator_c`/`rosidl_runtime_c`, compiled + round-tripped against the real TickLE codec in `tests/test_ros2_adapter.py`) - not yet checked against a *real* ROS 2 install's actual header shapes. (b) ⬜ the CMake package + macro wrapping this - explicit-call convention chosen over automatic `rosidl_generate_interfaces` extension-point registration for the first cut (that mechanism is undocumented rosidl/ament_cmake build-system internals this tool's own dev environment - no local ROS 2 at all - can't verify without many CI round trips; a caller's `CMakeLists.txt` calls `rosidl_typesupport_tickle_c_generate_interfaces(pkg, same-file-list-as-rosidl_generate_interfaces)` explicitly instead). Needs a real ROS 2 CI environment to iterate on (`ros-tooling/setup-ros`, already used by `rmw_tickle`'s own CI). (c) ⬜ automatic extension-point registration, deferred/stretch goal once (b) is proven. Wraps the resulting encode/decode in a `rosidl_message_type_support_t`/`rosidl_service_type_support_t` whose own private callback-struct shape only `rmw_tickle` itself needs to agree with (rosidl's typesupport contract never inspects it). "Supported subset" (above) written into this PLAN.md | none (parallel with 0) | 🔶 (a) done |
