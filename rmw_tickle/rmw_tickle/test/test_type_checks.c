...
int main(void) {
    ...
    // size are both accepted.
    msg_callbacks.struct_size = sizeof(rosidl_typesupport_tickle_c_message_callbacks_t) + 8;
    assert(NULL == rmw_create_publisher(node, &msg_handle, "/type_checks", &q, &pub_opts));
    assert(error_mentions("generated by a different rosidl_typesupport_tickle_c"));
    msg_callbacks.struct_size = sizeof(rosidl_typesupport_tickle_c_message_callbacks_t);
