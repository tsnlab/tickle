...
#define U(x) ((unsigned long long)(x))
    ...
    printf("RSTATS: role=%s acknack_tx immediate=%llu timer=%llu sent=%llu bits_sent=%llu\n", role,
           U(s.acknack_immediate), U(s.acknack_timer), U(s.acknack_sent), U(s.acknack_bits_sent));
