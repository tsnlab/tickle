...
int32_t BenchData_encode(struct BenchData* data, uint8_t* payload, uint32_t len) {
    ...
    {
        if ((uint32_t)encoded + 2788 > len) {
            return -1;
        }
        memcpy(payload + encoded, data->payload, 2788);
        encoded += 2788;
    }
    return encoded;
}
...
int32_t BenchData_decode(struct BenchData* data, const uint8_t* payload, uint32_t len, bool is_native_endian) {
    ...
    {
        if ((uint32_t)decoded + 2788 > len) {
            return -1;
        }
        memcpy(data->payload, payload + decoded, 2788);
        decoded += 2788;
    }
    return decoded;
}
