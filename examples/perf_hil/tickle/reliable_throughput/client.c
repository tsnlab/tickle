...
static void send_one(struct tt_Node* node, uint64_t time, void* param) {
    ...
    BenchCpuPlace_sample(&g_cpu_place, time, cpu_freq_period_ns);
    if (!have_pending) {
        pending_msg.seq = ++seq;
...
static void parse_args(int argc, char** argv) {
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "-i") == 0 && i + 1 < argc) {
            ...
            // The teardown drain cap, for a diagnostic run only (2026-09-26 - see the FastDDS twin);
            // the same letter in all three harnesses, and drain_cap_s= in every RESULT line.
            drain_s = atof(argv[++i]);
