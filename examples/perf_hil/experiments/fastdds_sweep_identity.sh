...
EXACT="deadline_miss_detection durability_late_join history_depth_burst_loss lifespan_expiry liveliness_loss_detection"
TIMING="reliable_latency reliable_throughput"
ALL="$EXACT $TIMING"
OUT=${OUT:-/tmp/fdds_identity.txt}
...
arm() {
    local tag="$1" only="${2:-all}" s r
    say ""
    if [ "$only" != timing ]; then
        for s in $EXACT; do
            say "$tag EXACT $s | $(run1 "$s" "-d 5" | sed -E 's/RESULT: framework=[a-z]+ scenario=[a-z_0-9]+ //')"
        done
    fi
    for s in $TIMING; do
        local args="-d 5"; [ "$s" = reliable_latency ] && args="-i 0.1 -d 10"
        for r in 1 2 3; do
            say "$tag TIME  $s rep$r | $(run1 "$s" "$args" | grep -oE '(role|sent|recv|rtt_avg_ms|rtt_max_ms|send_mbps|loss_pct|lost)=[a-z0-9.-]+' | tr '\n' ' ')"
        done
    done
}
build "$PRE";  arm "PRE1 "
build "$POST"; arm "POST "
build "$PRE";  arm "PRE2 " timing
say ""; say "=== done ==="
