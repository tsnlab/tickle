...
For every TickLE datagram (magic "KT"/"TK") the bytes on the wire are split into:
  l2l3l4        Ethernet + IPv4 + UDP headers, counted as 42 per datagram (outside TickLE's control)
  tt_header     4 per datagram
  sub_header    4 per submessage
  data_header   DATA's 20, FRAG_FIRST's 21, FRAG_CONT's 10 (user endpoints)
...
def inventory(path):
    ...
    for linktype, buf in packets(path):
        ...
        if not got:
            continue
        p, full = got
        if len(p) < 4 or p[:2] not in (b"KT", b"TK"):
            continue
        e = "<" if p[:2] == b"KT" else ">"
        src = p[3]
        n["datagrams"] += 1
        ...
        while off + 4 <= len(p):
            ...
            if endpoint == 0 and typ in (DATA, FRAG_FIRST, FRAG_CONT, HEARTBEAT, ACKNACK):
                ...
                n["discovery_sub"] += 1
            elif typ in (DATA, FRAG_FIRST) and len(body) >= 20:
                hlen = 20 if typ == DATA else 21
                b["data_header"] += hlen
