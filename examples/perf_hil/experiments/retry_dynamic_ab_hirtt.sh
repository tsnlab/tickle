#!/usr/bin/env bash
# BENEFIT arm. Does the dynamic retry interval (tt_RELIABLE_RETRY_INTERVAL=0, TickLE Dev 871129d8) make the
...
#
# This is the BENEFIT arm the c5 gate could not supply. c5 is the healthy case where the fixed 1 ms
# was already about right, and the gate fired there (+0.62% wire bytes, outside the spread). The
# regime the change exists for is one where a recovery takes much longer than 1 ms, and the P4 case
# that showed it is deferred by the user - so this reaches the same regime without fragmentation:
# a 10 ms delay with 5% loss. A recovery there costs ~20 ms, so a fixed 1 ms timer fires about
# twenty times before any recovery can land, and every firing re-requests a sample already in
# flight. Dynamic should learn ~20 ms and be clamped by the 10 ms ceiling.
#
# HOW TO READ IT, written before running:
#   BENEFIT - dynamic must send FEWER wire bytes per sample than fixed, outside the spread of 3.
#     If it does not, the change has no measured benefit anywhere and the default should stay at
#     1 ms regardless of what it costs at c5.
#   The c1 control repeats unchanged: no loss means no recoveries, so both builds must match.
...
OUT=${OUT:-/tmp/retry_ab_benefit.txt}
...
tc_set() { case "$1" in
    ...
    on)  sh_ "$CLIENT" "sudo -n tc qdisc replace dev eth0 root netem delay 10ms 2ms loss 5%" ;;
  esac; }
server_pids() {
...
build 0
say ""; run_cell "A  E hi-rtt fixed  " on  1000000
say ""; run_cell "C  G c1 fixed  " off 1000000
build 1
say ""; run_cell "B  F hi-rtt dynamic" on  0
say ""; run_cell "D  H c1 dynamic" off 0
tc_set off
say ""
