...
#
# A clean exit on either side doesn't prove any packet actually arrived - but unlike run_pair.sh's
# QEMU pairs (where both purpose-built role mains log per-message on both sides), these are the
# real, unmodified example binaries, and only one side of each pair actually logs real evidence:
# ping.c logs every reply it receives, but pong.c never logs per-request; subscriber.c's callback
# logs every message it decodes, but publisher.c only logs on error. So exactly one of
# $sender_grep/$receiver_grep is expected to be non-empty per call - whichever side's log this
# pair's real evidence lives in - and only that one is checked against at least $MIN_COUNT
# matches. (Passing both, or neither, would also work mechanically, just isn't needed today.)
run_round_trip() {
    sender=$1
    sender_args=$2
    receiver=$3
